/**
 * @file Node to handle EWIO2 digital outputs.
 * @author Metz Connect
 */

/**
 * ewio2 digital output node namespace.
 * @namespace DigitalOutNode
 */

module.exports = function(RED) {
    /**
    * Stores settings from digital OUT configuration menu.
    * Checks if all required settings are available, otherwise shows error status.
    * If a input message is received, connection to EWIO2 is established and digtial OUT is set.
    ...
    * @param {Object} config - digital OUT settings from digital OUT menu (frontend).
    */
    function EwioDigitalOutputsNode(config) {
        ...
        if(this.ewio2 === "") {
            ...
            if(!configNode) {
                ...
                function showStatus(status) {
                    log(node, 'DO recv ioport: ' + status.addr + '; ioport: ' + this.ioportDo + '; node.id: ' + node.id + '; this.ewio2: ' + this.ewio2 + '; status.configNodeId: ' + status.configNodeId + "; msg: " + status.message);
                ...
                // handles digital OUT errors received from pubSub in ewio2Connection.js and shows status by calling showStatus fct above (in digital-outputs.js)
                function showStatusDo(status) {
                    ...
                    if (status.addr && config.ioportDo) {
                        if (status.addr === config.ioportDo) {
                            showStatus(status);
                        }
                    }
                }
                pubSub.subscribe("show-status-do", showStatusDo);
                ...
                node.on('input', async function(msg, send, done) {
                    ...
                    if (typeof msg.payload === "number" || typeof msg.payload === "boolean") {
                        ...
                        const settedValue = await setValue(configData, msg.payload, "do", this.ioportDo, node.id, RED, "setValue", configNodeTls);
                        log(node, 'do setValue ' + settedValue + ' DONE ' + node.id);
                        // on error (undefined) --> send undefinded. Otherwise send boolean value
                        var msg = { payload: settedValue ? (settedValue === "1") : settedValue };
                        if (node.outputTopic) {
                            msg = { payload: settedValue ? (settedValue === "1") : settedValue, topic: node.outputTopic };
                        }
                        node.send(msg);
                    }
                    else {
                        log(node, 'msg.payload WRONG TYPE');
                        pubSub.publish("show-status-do", {"color": "red", "shape": "ring", "message": "@metz-connect/node-red-ewio2/ewio2:status.noBoolOrNumber", "configNodeId": this.ewio2, "addr": this.ioportDo});
                    }
                    if (done) {
                        done();
                    }
                ...
                node.on("close", function(done) {
                    ...
                    pubSub.unsubscribe("show-status-do");
    ...
    RED.nodes.registerType("EWIO2 - Digital OUT",EwioDigitalOutputsNode);
}
